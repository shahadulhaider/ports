...
import (
	...
	"github.com/shahadulhaider/ports/internal/scanner"
)

// Message types
type portsMsg []scanner.PortInfo
type tickMsg time.Time
type statusClearMsg struct{ gen int }

type model struct {
	table         table.Model
	allPorts      []scanner.PortInfo
...
func NewModel(initialPort int) tea.Model {
	cols := []table.Column{
...
func fetchPortsCmd(proto int) tea.Cmd {
	return func() tea.Msg {
		...
		switch proto {
		case 1:
			ports, err = scanner.GetUDPPorts()
		case 2:
			tcpPorts, tcpErr := scanner.GetListeningPorts()
			udpPorts, udpErr := scanner.GetUDPPorts()
			if tcpErr != nil {
				err = tcpErr
			} else if udpErr != nil {
				err = udpErr
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case portsMsg:
		...
		m.prevPorts = []scanner.PortInfo(msg)
		m.allPorts = newPorts
...
func (m model) applyDisplayPipeline(ports []scanner.PortInfo) []scanner.PortInfo {
	sorted := sortPorts(ports, m.sortMode)
	var deduped []scanner.PortInfo
	if m.dedupEnabled {
...
func portsToRows(ports []scanner.PortInfo) []table.Row {
...
func filterPorts(ports []scanner.PortInfo, query string) []scanner.PortInfo {
	if query == "" {
		return ports
	}
	...
	for _, p := range ports {
		if strings.Contains(strings.ToLower(strconv.Itoa(p.Port)), q) ||
			...
			strings.Contains(strings.ToLower(scanner.ServiceName(p.Port)), q) {
...
func sortPorts(ports []scanner.PortInfo, mode int) []scanner.PortInfo {
...
func dedupPorts(ports []scanner.PortInfo) []scanner.PortInfo {
